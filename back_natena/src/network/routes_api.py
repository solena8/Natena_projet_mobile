...
from src.config.database import Session, engine
...
def get_db():
    try:
        conn = sqlite3.connect("surfdatabase.db")
...
# Route pour obtenir la liste des spots avec sql
# @app.get("/all")
...
# route pour obtenir la liste de spots en utilisant l'orm:
@app.get("/all")
def get_spot_list_data():
    try:
                ...
                "spots": [
                    {
                        ...
                        "url": spot.images[0].url if spot.images else None,
                        "main": 1
                    } for spot in spots
                ]
            }
...
# route pour obtenir un spot en utilisant la requête sql:
...
# route pour obtenir un spot en utilisant l'orm:
@app.get("/spot/{id}")
def get_details_for_a_spot(id: int):
    try:
        with Session() as session:
            ...
            latitude, longitude = SpotDto.convertGeocode(spot.geocode)
